...
export default function KnowledgeBasePage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Удалить документ "${title}" и все связанные векторные фрагменты?`)) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setActionMessage(`Документ "${title}" и векторы успешно удалены.`);
  ...
  const handleReindex = async (id: string) => {
    try {
      setActionMessage("Переиндексация фрагментов через Gemini Embedding...");
      const res = await fetch(`/api/documents/${id}/reindex?tenant_id=tenant-demo-acme`, {
        method: "POST",
      });
      const data = await res.json();
      setActionMessage(data.message || "Успешно переиндексировано.");
      setTimeout(() => setActionMessage(""), 4000);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewDocTitle("");
      setNewDocContent("");
      setActionMessage(data.message || "Документ добавлен и проиндексирован!");
      setTimeout(() => setActionMessage(""), 4000);
    } catch (err: any) {
      alert(err.message);
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Plan Limit Progress Bar (Spec Section 5.1) */}
      <div className="p-4 rounded-2xl bg-white dark:bg-[#161922] border border-gray-200 dark:border-gray-800 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex-1 w-full max-w-lg">
          <div className="flex items-center justify-between text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">
            <span>Использование лимита документов (Тариф Team)</span>
            <span>{documents.length} из 50 документов</span>
          </div>
          <div className="w-full h-2 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden">
            <div
              className="h-full bg-indigo-600 rounded-full transition-all duration-500"
              style={{ width: `${Math.min(100, (documents.length / 50) * 100)}%` }}
            />
          </div>
        </div>
        ...
        >
          Снять ограничения с тарифом Scale →
        </a>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50/80 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">
                <tr>
                  <th className="py-3.5 px-6">Название документа</th>
                  <th className="py-3.5 px-6">Тип</th>
                  <th className="py-3.5 px-6">Статус</th>
                  <th className="py-3.5 px-6">Фрагменты (Chunks)</th>
                  <th className="py-3.5 px-6">Дата загрузки</th>
      ...
      {uploadModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#161922] border border-gray-200 dark:border-gray-800 rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Например: Правила компенсации такси и командировок 2026"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Вставьте текст регламента или политики компании. Текст будет автоматически разбит на смысловые фрагменты (chunks) и векторизован с помощью Google Gemini..."
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      ...
                      <span>Разбить и индексировать</span>
                    </>
                  )}
                </button>
              </div>
      ...
      {/* MODAL 2: Notion Connect Flow */}
      {notionModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#161922] border border-gray-200 dark:border-gray-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800">
              <h3 className="font-heading font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">
                <span className="w-6 h-6 rounded bg-black text-white flex items-center justify-center text-xs font-bold">
                  N
                </span>
                Подключение Notion Workspace
              </h3>
              <button
            ...
            <div className="pt-3 flex items-center justify-end gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  setActionMessage("Синхронизация с Notion Workspace успешно настроена.");
                  setTimeout(() => setActionMessage(""), 4000);
                }}
      ...
      {/* MODAL 3: View Chunks Preview */}
      {selectedDocChunks && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#161922] border border-gray-200 dark:border-gray-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-800 shrink-0">
              <div>
                <h3 className="font-heading font-bold text-base text-gray-900 dark:text-white">
                  Векторные фрагменты: {selectedDocChunks.title}
                </h3>
                <span className="text-xs text-gray-500">
                  Всего {selectedDocChunks.chunks?.length || 0} фрагментов с изоляцией tenant_id
                </span>
              </div>
              <button
